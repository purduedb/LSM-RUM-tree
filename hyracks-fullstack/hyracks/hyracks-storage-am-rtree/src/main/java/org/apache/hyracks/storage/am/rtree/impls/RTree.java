...
// import org.apache.hyracks.storage.am.common.updatememo;
...
public class RTree extends AbstractTreeIndex {
    ...
    public String printTree(IRTreeLeafFrame leafFrame, IRTreeInteriorFrame interiorFrame,
        ...
        //        int nextPageId = rootPage;
        //        LOGGER.error("->! " + nextPageId);
        //        for(int i = 0; i < 13; i++) {
        //        	nextPageId = nextPageId(nextPageId, interiorFrame, leafFrame, cmp);
        //        	LOGGER.error("-> " + nextPageId);
        //        }
    ...
    public int vacuumClean(int pageId, IRTreeLeafFrame leafFrame, IRTreeInteriorFrame interiorFrame,
        ...
        //        LOGGER.error("->! " + nextPageId);
        //        for(int i = 0; i < 13; i++) {
        //        	nextPageId = nextPageId(nextPageId, interiorFrame, leafFrame, cmp, memo);
        //        	LOGGER.error("-> " + nextPageId);
        //        }
    ...
    //    public int vacuumClean(int pageId, IRTreeLeafFrame leafFrame, IRTreeInteriorFrame interiorFrame, MultiComparator cmp) {
    //    	return cleanNextPage(pageId, interiorFrame, leafFrame, cmp); 
    //    }
    //    
    //    public int cleanNextPage(int pageId, IRTreeInteriorFrame interiorFrame, IRTreeLeafFrame leafFrame, 
    //    		MultiComparator cmp) throws Exception{
    //    	
    //    }
    ...
    public int cleanNextPage(int pageId, IRTreeInteriorFrame interiorFrame, IRTreeLeafFrame leafFrame,
        ...
        boolean current_released = false;
        if (node != null) {
    ...
    public int cleanFirstPage(int pageId, IRTreeInteriorFrame interiorFrame, IRTreeLeafFrame leafFrame,
        ...
        if (interiorFrame.isLeaf()) {
            ...
            node.releaseReadLatch();
            returnId = cleanFirstPage(((RTreeNSMInteriorFrame) (interiorFrame)).getFirstChild(cmp), interiorFrame,
                    leafFrame, cmp, memo);
        }
    ...
    private ICachedPage findLeaf(RTreeOpContext ctx) throws HyracksDataException {
        ...
        //Libin: debug log
        //HashMap<Integer, Integer> latch_logs = new HashMap<>();// 0 for released read, 1 for released write, 2 for read lock, 3 for right lock
        //int old_pid = 0;
        try {
    ...
    private void insertTuple(ICachedPage node, int pageId, ITupleReference tuple, RTreeOpContext ctx, boolean isLeaf)
        ...
        if (!isLeaf) {
            ...
            //            LOGGER.error("count: " + ((RTreeNSMLeafFrame)ctx.getLeafFrame()).getNumTupleCount());
        }
    ...
    private void insertTuple(ICachedPage node, int pageId, ITupleReference tuple, RTreeOpContext ctx, boolean isLeaf,
            UpdateMemo<Integer> memo) throws HyracksDataException {
        boolean succeeded = false;
        FrameOpSpaceStatus spaceStatus;
        //        LOGGER.error(pageId);
        if (!isLeaf) {
        ...
        //        LOGGER.error("LeafFrame: " + System.identityHashCode(ctx.getLeafFrame()));
    ...
    public class RTreeAccessor implements ITreeIndexAccessor {
        ...
        private AtomicInteger vacuumCount;
        ...
        public RTreeAccessor(RTree rtree, IIndexAccessParameters iap) {
            ...
            this.iap = iap;
            //this.vacuumCount = 0;
            this.vacuumCount = new AtomicInteger(0);
            this.cleanedPage = rootPage;
            //System.out.println("vacuum threshold is "+vacuumThreshold);
        }
        ...
        public void vacuumClean(UpdateMemo<Integer> memo) {
            ...
            try {
                this.cleanedPage =
                        rtree.vacuumClean(this.cleanedPage, ctx.getLeafFrame(), ctx.getInteriorFrame(), memo);
            } catch (Exception e) {
                // TODO Auto-generated catch block
                e.printStackTrace();
            }
        ...
        public void insertVacuumClean(ITupleReference tuple, UpdateMemo<Integer> memo) throws HyracksDataException {
            ...
            insert(tuple, ctx);
            //vacuumCount++;
            int result = vacuumCount.incrementAndGet();
            /*if (vacuumCount == vacuumThreshold) {
                vacuumCount = 0;
                vacuumClean(memo);
            }*/
            if (result % vacuumThreshold == 0) {
                vacuumClean(memo);
            }
        }
        ...
        public void insertCleanVacuumClean(ITupleReference tuple, UpdateMemo<Integer> memo)
                throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
            insertClean(tuple, ctx, memo);
            /*vacuumCount++;
        ...
        private void insert(ITupleReference tuple, IIndexOperationContext ictx) throws HyracksDataException {
            ...
            try {
                ...
                //                LOGGER.error("Leafnode: " + System.identityHashCode(leafNode));
                int pageId = ctx.getPathList().getLastPageId();
                ctx.getPathList().moveLast();
