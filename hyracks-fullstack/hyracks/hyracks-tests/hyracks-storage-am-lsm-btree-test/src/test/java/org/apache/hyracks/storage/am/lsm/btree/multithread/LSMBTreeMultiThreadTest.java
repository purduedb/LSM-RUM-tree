...
public class LSMBTreeMultiThreadTest extends OrderedIndexMultiThreadTest {
    ...
    @Override
    protected ITreeIndex createSecondaryTreeIndex(ITypeTraits[] typeTraits, IBinaryComparatorFactory[] cmpFactories,
        ...
        return createSecondaryTreeIndex(harness, typeTraits, cmpFactories, bloomFilterKeyFields, filterTypeTraits,
                filterCmpFactories, btreeFields, filterFields);
    }

    public static LSMBTree createSecondaryTreeIndex(LSMBTreeTestHarness harness, ITypeTraits[] typeTraits,
        ...
        return LSMBTreeUtil.createLSMSecondaryTree(harness.getIOManager(), harness.getVirtualBufferCaches(),
                harness.getFileReference(), harness.getDiskBufferCache(), typeTraits, cmpFactories,
                bloomFilterKeyFields, harness.getBoomFilterFalsePositiveRate(), harness.getMergePolicy(),
                harness.getOperationTracker(), harness.getIOScheduler(), harness.getIOOperationCallbackFactory(),
                harness.getPageWriteCallbackFactory(), false, filterTypeTraits, filterCmpFactories, btreeFields,
    ...
    protected ArrayList<TestWorkloadConf> getTestWorkloadConf() {
        ...
        //	      Insert, update only workload.
        TestOperation[] insertOnlyOps = new TestOperation[] { TestOperation.INSERT, TestOperation.UPDATE };
        workloadConfs
