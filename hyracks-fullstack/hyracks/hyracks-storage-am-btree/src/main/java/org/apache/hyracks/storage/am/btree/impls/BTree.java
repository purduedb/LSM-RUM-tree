...
public class BTree extends AbstractTreeIndex {
    ...
    private boolean insertLeafClean(ITupleReference tuple, int targetTupleIndex, int pageId, BTreeOpContext ctx,
    ...
    public int cleanFirstPage(int pageId, IBTreeInteriorFrame interiorFrame, IBTreeLeafFrame leafFrame,
        ...
        if (interiorFrame.isLeaf()) {
            ...
            returnId = cleanFirstPage(((BTreeNSMInteriorFrame) (interiorFrame)).getLeftmostChildPageId(), interiorFrame,
                    leafFrame, cmp, memo);
        }
    ...
    private void performOpClean(int pageId, ICachedPage parent, boolean parentIsReadLatched, BTreeOpContext ctx,
    ...
    public class BTreeAccessor implements ITreeIndexAccessor {
        ...
        public void secondaryInsert(ITupleReference tuple, ITupleAcceptor acceptor) throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
        ...
        public void secondaryInsertCleanVacuumClean(ITupleReference tuple, UpdateMemo<Integer> memo,
                ITupleAcceptor acceptor) throws HyracksDataException {
            ctx.setOperation(IndexOperation.INSERT);
            //            ctx.setAcceptor(acceptor);
            insertClean(tuple, ctx, memo);
            if (vacuumCount == vacuumThreshold) {
        ...
        public void vacuumClean(UpdateMemo<Integer> memo) {
            ...
            try {
                this.cleanedPage =
                        btree.vacuumClean(this.cleanedPage, ctx.getLeafFrame(), ctx.getInteriorFrame(), memo);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        private void insertClean(ITupleReference tuple, BTreeOpContext ctx, UpdateMemo<Integer> memo)
                throws HyracksDataException {
            ctx.getModificationCallback().before(tuple);
        ...
        private void insertUpdateOrDeleteClean(ITupleReference tuple, BTreeOpContext ctx, UpdateMemo<Integer> memo)
                throws HyracksDataException {
            ctx.reset();
            ctx.getPred().setLowKeyComparator(ctx.getCmp());
            ctx.getPred().setHighKeyComparator(ctx.getCmp());
