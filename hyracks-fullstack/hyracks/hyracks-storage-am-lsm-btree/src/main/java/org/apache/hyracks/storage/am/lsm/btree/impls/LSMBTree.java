                ...
                if (ctx.getOperation() == IndexOperation.UPDATE) {
                    if (UpdateMemoConfig.CLEAN_UPON_UPDATE) {
                        if (UpdateMemoConfig.VACUUM_CLEAN) {
                            ctx.getCurrentMutableBTreeAccessor().secondaryInsertCleanVacuumClean(indexTuple, globalUM,
                                    AntimatterAwareTupleAcceptor.INSTANCE);
                        } else {
                            ctx.getCurrentMutableBTreeAccessor().secondaryInsertClean(indexTuple, globalUM,
                                    AntimatterAwareTupleAcceptor.INSTANCE);
                        }
                    } else {
                        if (UpdateMemoConfig.VACUUM_CLEAN) {
                            ctx.getCurrentMutableBTreeAccessor().secondaryInsertVacuumClean(indexTuple, globalUM,
                                    AntimatterAwareTupleAcceptor.INSTANCE);
                        } else {
                            ctx.getCurrentMutableBTreeAccessor().secondaryInsert(indexTuple,
                                    AntimatterAwareTupleAcceptor.INSTANCE);
                            //                    		insert(indexTuple, ctx);
                        }
                    }
            ...
            try {
                ...
                while (scanCursor.hasNext()) {
                    ...
                    if (UpdateMemoConfig.CLEAN_UPON_FLUSHING) {
                        ...
                        boolean isOld = globalUM.cleanEntity(
                                ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
                                ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
                ...
                try {
                    ...
                    while (cursor.hasNext()) {
                        ...
                        if (UpdateMemoConfig.CLEAN_UPON_MERGING) {
                            ...
                            boolean isOld = globalUM.cleanEntity(
                                    ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki))
                                            .getInt(),
                                    ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti))
                                            .getInt());
