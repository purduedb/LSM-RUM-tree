...
public abstract class AbstractSlotManager implements ISlotManager {
    ...
    public int getSlotEndOff() {
        int debug_val = frame.getTupleCount();
        int result = frame.getBuffer().capacity() - (frame.getTupleCount() * slotSize);
        /*if(result>2048){
            System.out.println("error");
        }*/
        result = Math.min(result, 2048);
        return result;//frame.getBuffer().capacity() - (frame.getTupleCount() * slotSize);
    }
