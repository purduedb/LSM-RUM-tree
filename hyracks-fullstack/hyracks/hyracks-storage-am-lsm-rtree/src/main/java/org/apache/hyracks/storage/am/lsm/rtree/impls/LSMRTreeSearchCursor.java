            ...
            while (rtreeCursors[currentCursor].hasNext()) {
                ...
                if (lsmHarness.useLocalUM()) { // local UM
                    ...
                    if (tComp.getLocalUM().hasNewerEntity(
                            ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ki), currentTuple.getFieldLength(ki))
                                    .getInt(),
                            ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ti), currentTuple.getFieldLength(ti))
                                    .getInt())) {
                        killerTupleFound = true;
                    } else {
                        if (lsmHarness.getGlobalUM().hasNewerEntity(
                                ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ki),
                                        currentTuple.getFieldLength(ki)).getInt(),
                                ByteBuffer.wrap(tupleData, currentTuple.getFieldStart(ti),
                                        currentTuple.getFieldLength(ti)).getInt())) {
                            killerTupleFound = true;
                        }
                    }
                } else { // global UM
