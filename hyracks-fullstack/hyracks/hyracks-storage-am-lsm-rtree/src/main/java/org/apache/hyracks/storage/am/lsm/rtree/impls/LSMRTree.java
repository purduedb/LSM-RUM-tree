...
public class LSMRTree extends AbstractLSMRTree {
    ...
    //    public static final boolean useLocalUM = true;
    ...
    public ILSMDiskComponent doFlush(ILSMIOOperation operation) throws HyracksDataException {
        //    	long flushStart = 0L;
        //    	long sortStart = 0L;
        //    	long flushDiskStart = 0L;
        //    	long flushEnd = 0L;
        //    	long sortEnd = 0L;
        //    	long flushDiskEnd = 0L;
        LOGGER.warn("FLUSH!!!!!!!!!!!");
        ...
        try {
            try {
                ...
                //                flushDiskStart = System.currentTimeMillis();
                ...
                //                flushDiskEnd = System.currentTimeMillis();

                if (isLocalUM()) {
                    //                	long start = System.nanoTime();
                    mergeUMLocalToGlobal(flushingComponent);
                    //                	long end = System.nanoTime();
                    //                	LOGGER.error((end-start) / 1000);
                }
            } finally {
                try {
                    ...
                    if (USE_LOCAL_UM) {
                        //                    	LOGGER.warn("FLUSHING- localUMSize: " + flushingComponent.getLocalUM().getSize()  + " / total: " + this.globalUM.getSize());
                        flushingComponent.getLocalUM().reset();
                    } else {
                        //                    	LOGGER.warn("FLUSHING- total: " + this.globalUM.getSize());
                    }
                }
            }
        ...
        //        flushEnd = System.currentTimeMillis();

        //        long flushTotal = flushEnd - flushStart;
        //        long flushSort = sortEnd - sortStart;
        //        long flushDisk = flushDiskEnd - flushDiskStart;
        //        
        //        LOGGER.error("              Total: " + flushTotal + " ms , sort/filter: " + flushSort + " ms , flush: " + flushDisk);
    ...
    private void mergeUMLocalToGlobal(LSMRTreeMemoryComponent flushingComponent) {
        //    	local
        //    	UpdateMemo<Double> localUM = flushingComponent.getLocalUM();
    ...
    private boolean fill(RTreeSearchCursor rtreeScanCursor, TreeTupleSorter rTreeTupleSorter)
        ...
        //            LOGGER.error("=FFF--------> flushed: " + i + ", UM size: " + globalUM.getSize());
        return isEmpty;
    }

    private boolean filteredFill(LSMRTreeMemoryComponent flushingComponent, RTreeSearchCursor rtreeScanCursor,
            TreeTupleSorter rTreeTupleSorter) throws HyracksDataException {
        boolean isEmpty = true;
        //        int i = 0;
        //        int j = 0;
        while (rtreeScanCursor.hasNext()) {
            ...
            boolean isOld;
            if (USE_LOCAL_UM) {
                isOld = flushingComponent.getLocalUM().cleanEntity(
                ...
                //        		LOGGER.warn("local: " + tComp.getLocalUM().getSize() + " / total: " + this.globalUM.getSize());
            } else {
                isOld = globalUM.cleanEntity(
                        ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
                        ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
            }
            ...
            if (!isOld) {
                ...
                rTreeTupleSorter.insertTupleEntry(rtreeScanCursor.getPageId(), rtreeScanCursor.getTupleOffset());
                //        		i++;
                //        		if(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() == 37
                //        				 || ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() == 3) {
                //        			LOGGER.error("=> flushed- key: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() 
                //            				+ ", ts: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
                //        		}
            } else {
                //        		j++;
                //        		if(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() == 37
                //       				 || ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() == 3) {
                //        			LOGGER.error("=> filtered- key: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() 
                //            				+ ", ts: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
                //        		}
            }
        }
        //        LOGGER.error("=FFF-------> flushed: " + i + " filtered: " + j + ", UM size: " + globalUM.getSize());
        return isEmpty;
    }
    ...
    public ILSMDiskComponent doMerge(ILSMIOOperation operation) throws HyracksDataException {
        //    	long mergeStart = 0L;
        //    	long loadMergeStart = 0L;
        //    	long mergeEnd = 0L;
        //    	long loadMergeEnd = 0L;
        //    	LOGGER.error("MERGE!!!!!!!!!!!");
        ...
        try {
            ...
            //            loadMergeStart = System.currentTimeMillis();
            ...
            //            loadMergeEnd = System.currentTimeMillis();
        ...
        //        long loadMerge = loadMergeEnd - loadMergeStart;
        //        
        //        LOGGER.error("              Total: " + mergeTotal + " ms , load/merge/filter: " + loadMerge +  " ms");
        long end = System.currentTimeMillis();
    ...
    private ILSMDiskComponentBulkLoader loadMergeBulkLoader(LSMRTreeMergeOperation mergeOp, IIndexCursor cursor,
        ...
        try {
            try {
                ...
                LOGGER.error("=MMM-------> merged: " + i + ", UM size: " + globalUM.getSize());
            } finally {
                cursor.close();
            }
            abort = false;
        ...
        try {
            try {
                ...
                while (cursor.hasNext()) {
                    ...
                    boolean isOld;
                    if (USE_LOCAL_UM) {
                        isOld = tComp.getLocalUM().cleanEntity(
                                ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
                                ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
                    ...
                    if (!isOld) {
                        ...
                        i++;
                        //                		if(ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() == 37) {
                        //                			LOGGER.error("=> merged- key: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt() 
                        //                    				+ ", ts: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
                        //                		}
                    } else {
                        j++;
                ...
                LOGGER.error("=MMM-------> merged: " + i + " filtered: " + j + ", UM size: " + globalUM.getSize());
            } finally {
                cursor.close();
            }
            abort = false;
    ...
    public void modify(IIndexOperationContext ictx, ITupleReference tuple) throws HyracksDataException {
        ...
        if (ctx.getOperation() == IndexOperation.INSERT) {
            ...
            //	        	String line = "";
            //	        	
            //	        	for(int k = 0; k < tuple.getFieldCount(); k++) {
            //	        		line += ByteBuffer.wrap(tupleData, tuple.getFieldStart(k), tuple.getFieldLength(k)).getInt() + ", ";
            //	        	}
            //	        	LOGGER.warn(line);
            //        	}
            ...
            //        	String rec = TupleUtils.printTuple(indexTuple, fieldSerdes);
            //        		if (LOGGER.isWarnEnabled()) {
            //    			LOGGER.warn("INSERT " + rec);
            //  			}

            ctx.getCurrentMutableRTreeAccessor().insert(indexTuple);
        } else { // ctx.getOperation() == IndexOperation.DELETE
            //        	if(LOGGER.isInfoEnabled())
            //        		LOGGER.info("MODIFY" + " - " + ctx.getOperation());
            ...
            //        	if(LOGGER.isInfoEnabled())
            //        		LOGGER.info("key: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getDouble() 
            //        				+ ", ts: " + ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getDouble());
            //        	long start = System.nanoTime();
            if (USE_LOCAL_UM) {
                LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
                tComp.getLocalUM().insertOrUpdate(
                        ByteBuffer.wrap(tupleData, tuple.getFieldStart(ki), tuple.getFieldLength(ki)).getInt(),
                        ByteBuffer.wrap(tupleData, tuple.getFieldStart(ti), tuple.getFieldLength(ti)).getInt());
            ...
            //        	cnt++;
            //        	if(cnt % 1000 == 0) {
            ////        		LOGGER.error("cnt: " + cnt + "time: " + ((time) / cnt));
            //        	}

            //        	LOGGER.warn(ctx.getOperation());
            if (ctx.getOperation() == IndexOperation.UPDATE) {
                if (UpdateMemoConfig.CLEAN_UPON_UPDATE) {
                    if (UpdateMemoConfig.VACUUM_CLEAN) {
                        if (USE_LOCAL_UM) {
                            LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
                            ((RTreeAccessor) ctx.getCurrentMutableRTreeAccessor()).insertCleanVacuumClean(indexTuple,
                                    tComp.getLocalUM());
                        } else {
                            ((RTreeAccessor) ctx.getCurrentMutableRTreeAccessor()).insertCleanVacuumClean(indexTuple,
                                    globalUM);
                        ...
                        if (USE_LOCAL_UM) {
                            LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
                            ((RTreeAccessor) ctx.getCurrentMutableRTreeAccessor()).insertClean(indexTuple,
                                    tComp.getLocalUM());
                        } else {
                    ...
                    if (UpdateMemoConfig.VACUUM_CLEAN) {
                        if (USE_LOCAL_UM) {
                            LSMRTreeMemoryComponent tComp = (LSMRTreeMemoryComponent) ctx.getComponentHolder().get(0);
                            ((RTreeAccessor) ctx.getCurrentMutableRTreeAccessor()).insertVacuumClean(indexTuple,
                                    tComp.getLocalUM());
                        } else {
                            ((RTreeAccessor) ctx.getCurrentMutableRTreeAccessor()).insertVacuumClean(indexTuple,
                                    globalUM);
    ...
    //    public String printTree(ISerializerDeserializer[] keySerdes) {
