...
public class IndexMultiThreadTestDriver {
    ...
    public long[] run(int numThreads, int numRepeats, int numOps, int batchSize, String inputPrefix, int[] indexes,
            String method) throws InterruptedException, HyracksDataException {
        ...
        for (int i = 0; i < numRepeats; i++) {
            ...
            for (int j = 0; j < numThreads; j++) {
                workers[j] = workerFactory.create(dataGen, opSelector, index, threadNumBatches, j, numThreads,
                        inputPrefix, indexes, method);
