...
public class RTreeTestUtils extends TreeIndexTestUtils {
    ...
    public void insertDoubleTuples(IIndexTestContext ctx, int numTuples, Random rnd, boolean isPoint)
        ...
        //    	LOGGER.info("JW-RTreeTestUtils- " + numTuples + " insert");
        int fieldCount = ctx.getFieldCount();
        int numKeyFields = ctx.getKeyFieldCount();
        //        LOGGER.info(fieldCount + ", " + numKeyFields);
        ...
        for (int i = 0; i < numTuples; i++) {
            ...
            //            String line = "";
            //            for(int k = 0; k < ctx.getFieldCount(); k++) {
            //            	line += fieldValues[k] + ", ";
            //            }
            //            LOGGER.info(line);
            // Set values.
            ...
            //            String line = "";
            //            byte[] tupleData = ctx.getTuple().getFieldData(0);
            //            ArrayTupleReference tr = ctx.getTuple();
            //            for(int k = 0; k < ctx.getTuple().getFieldCount(); k++) {
            //            	line += ByteBuffer.wrap(tupleData, tr.getFieldStart(k), tr.getFieldLength(k)).getDouble() + ", ";
            //            }
            //            LOGGER.warn(line);
    ...
    public void updateDoubleTuples(IIndexTestContext ctx, int numTuples, Random rnd) throws HyracksDataException {
        updateDoubleTuples(ctx, numTuples, rnd, false);
    }

    @SuppressWarnings("unchecked")
    public void updateDoubleTuples(IIndexTestContext ctx, int numTuples, Random rnd, boolean isPoint)
            throws HyracksDataException {
        int fieldCount = ctx.getFieldCount();
        int numKeyFields = ctx.getKeyFieldCount();
        ...
        for (int i = 0; i < numTuples; i++) {
            ...
            try {
                ctx.getIndexAccessor().update(ctx.getTuple());
                ctx.insertCheckTuple(createDoubleCheckTuple(fieldValues, ctx.getKeyFieldCount()), ctx.getCheckTuples());
            } catch (HyracksDataException e) {
                // We set expected values only after insertion succeeds because
                // we ignore duplicate keys.
